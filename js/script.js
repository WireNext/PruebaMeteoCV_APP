...
function renderizar(data, nombre, targetId, aviso) {
    ...
    if (aviso) {
        ...
        alertaHtml = `<div class="alerta-card" style="background-color: ${aviso.color}; color: ${textColor}"><h4>${aviso.titulo}</h4><div class="alerta-desc">${aviso.desc}</div><a href="avisos.html" class="alerta-link" style="color: ${textColor}; border-color: ${textColor}">Llegir més</a></div>`;
    }
    ...
    for (let i = horaActual; i < horaActual + 24; i++) {
        html += `<div class="hora-item"><span class="hora-txt">${i === horaActual ? 'Ara' : (i % 24) + 'h'}</span><span class="hora-icon">${obtenerIcono(hourly.weather_code[i])}</span><span class="hora-temp">${Math.round(hourly.temperature_2m[i])}°</span></div>`;
    }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        html += `<div class="dia-fila"><span class="dia-nom">${i === 0 ? 'Hui' : diaNombre}</span><span class="dia-icon">${obtenerIcono(daily.weather_code[i])}</span><span class="dia-temps">${Math.round(daily.temperature_2m_max[i])}° <small>${Math.round(daily.temperature_2m_min[i])}°</small></span></div>`;
    }
    ...
    html += `</div></div><div class="detalles-grid">
            ...
            <div class="detalle-card"><h3>🌅 ALBA</h3><div class="detalle-valor">${formatHora(daily.sunrise[0])}</div></div>
            <div class="detalle-card"><h3>🌇 OCÀS</h3><div class="detalle-valor">${formatHora(daily.sunset[0])}</div></div>
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const ejecutar = () => {
        ...
        if (p) { localStorage.setItem("ultimPobleBuscat", p); buscarTiempo(p, "resultado-tiempo-home"); input.blur(); }
    };
    ...
    if (input) {
        ...
        try {
            ...
            const data = await res.json();
            data.municipis.forEach(m => { const o = document.createElement("option"); o.value = m; dl.appendChild(o); });
        } catch (e) {}
    }
